...
import { PhoneCall, ShieldAlert, Eye, Heart, Inbox } from "lucide-react";
    ...
    <AppShell role="biz" nav={BIZ_NAV} breadcrumb={["SENTINEL", "企业用户", "通话记录"]}>
      ...
      <div className="grid grid-cols-12 gap-5 mb-6">
          ...
          <div className="panel p-6">
            {list.loading && list.items.length === 0 ? (
              ...
              <div className="panel p-12 flex flex-col items-center justify-center text-center">
                ...
                <div className="font-display text-[calc(15px*var(--fz))] font-extrabold">暂无通话记录</div>
                <div className="mt-1 font-mono text-[calc(11px*var(--fz))] uppercase tracking-[0.14em] text-ink-soft font-bold">NO CALL HISTORY YET</div>
              </div>
            ) : (
              <DataTable<CallLog>
              rows={list.items}
              ...
              actions={(r) => (
                <button onClick={() => setActive(r)} className="font-mono text-[calc(11px*var(--fz))] font-bold text-indigo-deep hover:underline px-2">详情</button>
              )}
            />
            )}
          </div>
        ...
        <aside className="col-span-12 lg:col-span-4 space-y-4">
          <div className="panel p-6">
            <div className="font-mono text-[calc(10px*var(--fz))] uppercase tracking-[0.14em] text-ink-soft font-bold mb-2">STRATEGY</div>
            <div className="font-display text-[calc(16px*var(--fz))] font-extrabold mb-2">通话策略</div>
          ...
          <div className="panel p-6" style={{ background: "var(--indigo-soft)" }}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-display text-[calc(14px*var(--fz))] font-extrabold" style={{ color: "var(--indigo-deep)" }}>关怀模式说明</span>
            </div>
            <div className="text-[calc(12px*var(--fz))] font-medium leading-[1.7]" style={{ color: "var(--indigo-deep)" }}>
        ...
        {active && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <Mini label="时间"><span className="font-mono text-[calc(11px*var(--fz))]">{active.createdAt}</span></Mini>
            </div>
            <div className="panel p-5">
              <div className="font-mono text-[calc(10px*var(--fz))] uppercase tracking-[0.14em] text-ink-soft font-bold mb-2">VERDICT</div>
              <div className="flex items-center gap-3">
                <VerdictPill v={active.verdict} />
                <span className="text-[calc(14px*var(--fz))] font-medium text-ink-2">{active.reason}</span>
              </div>
            </div>
            <div className="panel p-5">
              <div className="font-mono text-[calc(10px*var(--fz))] uppercase tracking-[0.14em] text-ink-soft font-bold mb-3">ENGINE BREAKDOWN</div>
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <div key={e.k} className="p-3 rounded-2xl bg-canvas-2">
                    ...
                    <div className="mt-1 numplate text-[calc(18px*var(--fz))]">{e.v}</div>
                  </div>
                ))}
              </div>
            </div>
...
function VerdictPill({ v }: { v: CallLog["verdict"] }) {
  ...
  const m = map[v];
  return (
    <span className="font-mono text-[calc(10px*var(--fz))] uppercase tracking-[0.14em] px-2 py-1 rounded-full font-bold" style={{ background: m.soft, color: m.fg }}>
      {v}
    </span>
  );
}
