...
import { Inbox, Clock, CheckCircle2, XCircle, MessageSquareWarning, Flag } from "lucide-react";
...
export default function SysAppealsPage() {
  ...
  const decide = async (id: string, status: "已通过" | "已驳回") => {
    try {
      await api.appeals.setStatus(id, status);
      toast("success", status, `#${id.slice(-4)}`);
      list.refresh();
    } catch (e) {
  ...
  return (
    <AppShell role="sysadmin" userName="陈静" nav={SYSADMIN_NAV} breadcrumb={["SENTINEL", "系统管理员", "申诉处理"]}>
      <PageHeader
        ...
        title="申诉与举报处理"
        desc="家庭与企业用户提交的误判申诉与号码举报，统一在此审批。通过后将由平台同步至云端黑/白名单或解除标记。"
      />
