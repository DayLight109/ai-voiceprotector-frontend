// 全局共享的业务实体类型定义。
// 历史上这里还存放过 mock 种子数据（SEED），已随前后端对接完成移除；
// 现仅保留类型，供 lib/api.ts 及各页面引用。
...
export type ManagedUser = {
  id: string;
  name: string;
  // 与后端 users.role 枚举一致：family=家庭成员 / biz=员工 /
  // family_admin=家庭管理员 / admin=企业管理员 / sysadmin=系统管理员。
  // （旧 mock 体系的 viewer/operator/admin 后端不识别，提交会被 403）
